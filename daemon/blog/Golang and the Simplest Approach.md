...
The above program doesn't produce any errors; in particular you don't get any notification that using `accumulate()` as a goroutine means the return value is ignored--that value is quietly dropped on the ground. This has implications:
...
- The basic concept of a function is violated when it is used as a goroutine. Such functions no longer take inputs and produce an output, but do *everything* via side effects. If you write a function that might be used as a goroutine, you must write it differently. It is not as simple as saying "any function can be used as a goroutine." 
